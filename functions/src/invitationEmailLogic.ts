...
export function shouldNotifyInvitationAdmins(suppressAdminNotification: unknown): boolean {
  return suppressAdminNotification !== true;
}
...
export function buildInvitationEmail(input: InvitationEmailInput): InvitationEmail {
  ...
  const expiryDate = input.expiresAt && !Number.isNaN(input.expiresAt.getTime())
  ...
  const expiryText = expiryDate
    ? `This invitation expires on ${expiryDate} (UTC), seven days after it was created.`
    : "This invitation expires seven days after it was created.";
  const subject = `You're invited to join ${organizationName} on League Hub`;
  const text = [
    greeting,
    "",
    `You've been invited to join ${organizationName} as ${assignedRole}.`,
    "",
    ...
    "",
    `${expiryText} If you were not expecting it, you can ignore this email.`,
    "",
    "League Hub",
  ].join("\n");
